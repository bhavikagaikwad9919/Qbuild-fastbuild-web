...
import { clearAllStates } from "app/main/projects/store/projectsSlice";
...
export const setUserData = (data) => async (dispatch, getState) => {
  const user = {
    role: data.role ? data.role : data.department,
    from: data.SSO ? data.SSO : "email",
    data: {
      id: data._id,
...
export const pushToken = createAsyncThunk(
  ...
  async (pushToken) => {
    try {
      const response = await jwtService.pushToken(pushToken);
    } catch (e) {
      console.log(e);
    }
  }
...
export const changePassword = (form) => async (dispatch) => {
  try {
    let response = await jwtService.changePassword(form);
...
export const { setUser, routes, userLoggedOut } = userSlice.actions;
