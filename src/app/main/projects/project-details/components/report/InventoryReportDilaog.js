...
import { downloadInventoryExcelReport,
...
const InventoryReportDilaog = (props) => {
  ...
  const callInv = (newInv) =>{
    ...
    if(filterInv.length > 0){
      ...
      let newSupp = ['All'];
      inventoryList.forEach((inv)=>{
        inv.supplier.forEach((ins)=>{
          newSupp.push(ins)
        })
      })
      let uniqueSuppliers = newSupp.filter((item, i, ar) => ar.indexOf(item) === i);
...
export default React.memo(InventoryReportDilaog);
