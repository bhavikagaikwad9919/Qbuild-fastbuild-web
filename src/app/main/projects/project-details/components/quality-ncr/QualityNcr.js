...
import QualityNcrDialog from "./QualityNcrDialog";
import Typography from "@material-ui/core/Typography";
import moment from "moment";
import ViewNcr from "../safety-ncr/ViewNcr";
import { dispatchWarningMessage } from "app/utils/MessageDispatcher";
import { detailQualityData,  openNewDialog, openEditDialog, addEntryToSummary } from "app/main/projects/store/projectsSlice";
...
function QualityNcr(props) {
  const classes = useStyles(props);
  const dispatch = useDispatch();
  const loading = useSelector(({ projects }) => projects.loading);
  const qualityNcr = useSelector(({ projects }) => projects.qualityNcrs.qualityNcrsList);
  ...
  useEffect(() => {
    team.forEach((t)=>{
      ...
      {
         const member=t.tab_access.filter((i)=>i === "Quality NCR");
         console.log(member)
         if(member[0] === "Quality NCR")
         {
           setAccess(true)
         }
      }
  ...
  const openDialog = async (data) => {
    await dispatch(detailQualityData({qualityDataId : data._id}));
    dispatch(openEditDialog(data));
  }

  const viewNcrReport = (details) =>{
    let reportName = "Quality NCR", actionType = "View", title = `${details.refNo}`;
    dispatch(detailQualityData({qualityDataId : details._id})).then((response) => {
  ...
  const downloadDocument = (data) =>{
    if(data.ncrDoc.length > 0){
      ...
      link.click();
      let reportName = "Quality NCR", actionType = "Download", title = `${data.refNo}`;
      dispatch(addEntryToSummary ({ projectId, userId, reportName, actionType, title }));
    }else{
    ...
    <React.Fragment>
      <Paper className="w-full rounded-8 shadow-1">
        <div className="flex items-center justify-between px-16 h-64 border-b-1">
          <Typography className="text-16">Quality NCR</Typography>
        </div>
        ...
        {hide === true ? null :
          <FuseAnimate animation="transition.expandIn" delay={300}>
            <Fab
              ...
              }: () => dispatchWarningMessage(dispatch, "You don't have an access to add a issue.")}
            >
              <Icon>add</Icon>
            </Fab>
          </FuseAnimate>
