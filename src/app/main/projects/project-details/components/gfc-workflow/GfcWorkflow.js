...
const GfcWorkflow = (props) => {
  const classes = useStyles(props);
  const gfcFolders = useSelector(({ projects }) => projects.gfcFolders);
  ...
  useEffect(() => {
    team.forEach((t)=>{
      ...
      {
         const member=t.tab_access.filter((i)=>i === "Gfc Workflow");
         console.log(member)
         if(member[0] === "Gfc Workflow")
         {
           setAccess(true)
         }
      }
  ...
  const createFolder = () => {
     let doc = gfcFolders.filter(fold => fold.folderName.toLowerCase() === folder.name.toLowerCase());
     if(doc.length > 0)
     {
      ...
      setPageLoading(true);
      setAddOpen(false);
      dispatch(addGfcFolder({ projectId,folderName:folder.name })).then(
        (response) => {
          if (response) {
  ...
  const openFolder = async (value) =>{
    await dispatch(listGfcs({ projectId, folderId:value._id })).then(
      (response) => {
        setFolderOpen(true);
        setFolder(value)
      }
  ...
  const updateFolder = () => {
    let list = gfcFolders.filter(df=> df.folderName.toLowerCase() !== folderName.toLowerCase());
    ...
    setPageLoading(true);
    setAddOpen(false);
    dispatch(updateGfcFolder({ projectId,folderId:folderId,folderName:folder.name})).then(
      (response) => {
        if (response) {
          setPageLoading(false);
          setFolder({ name: '', type: ''});
  ...
  return (
    <div>
      ...
      {folderOpen ? 
        <GfcList data ={folder} close={onClose}/>
        :
       <>
         <div className="flex flex-1 flex-row w-full mb-12">
            <div className="flex w-full items-center justify-start gap-10" >
              <Typography className="text-16 font-bold">GFC Workflow</Typography>
            </div>
...
export default React.memo(GfcWorkflow);
